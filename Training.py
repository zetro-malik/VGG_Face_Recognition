...
class FaceExtractor(object):
    ...
    def extract_faces(self, video_file, save_folder):
        ...
        print("length: {}, w x h: {} x {}, fps: {}".format(
        ...
        while (cap.isOpened()):
            ...
            if ret:
                ...
                if len(faces) > 1:  # Get the largest face as main face
                    face = max(faces, key=lambda rectangle: (
                        rectangle[2] * rectangle[3]))  # area = w * h
                elif len(faces) == 1:
                    face = faces[0]
                if face is not None:
                    ...
                    imgfile = os.path.basename(video_file).replace(
                        ".", "_") + "_" + str(frame_counter) + ".png"
